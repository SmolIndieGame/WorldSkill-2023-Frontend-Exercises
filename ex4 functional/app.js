...
const chain = (computation, ...functions) => {
  return functions.length === 0
    ...
    : functions[functions.length - 1](
        chain(computation, ...functions.slice(0, -1))
      );
};
  ...
  len === 0
    ...
    : chain(
        ...
        (idx) =>
          predicate(array[idx])
            ? array[idx]
            : findRandom($swap(array, idx, len - 1), predicate, len - 1)
      );
        ...
        [
          ...
          findRandom(
            new Array(rows * columns)
              .fill(undefined)
              .map((_, i) => new Vector(Math.floor(i / columns), i % columns)),
            (vec) =>
...
const gameLoop = async (state, input) => {
  ...
  const newVelocity =
    ...
    (state.velocity.i === -tmpVelocity.i && state.velocity.j === -tmpVelocity.j)
      ? state.velocity
      : tmpVelocity;
...
function $display(snake, foods) {
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < columns; j++) {
      const cell = document.getElementById(Vector.toCellId(new Vector(i, j)));
      cell.classList = "cell empty-cell";
    }
  }
  snake.forEach(
  ...
  foods.forEach(
    (e) =>
      (document.getElementById(Vector.toCellId(e)).classList = "cell food-cell")
  );
}
